...
def get_connectivity_matrix(my_seed, N,rho,epsilon):
    ...
    S = symmetrize(np.random.uniform(-1,1,(N,N)))  #(2*np.random.rand(N, N) - np.ones((N,N)))
    np.fill_diagonal(S,0)
    sparseOld(S, rho)
    A = antisymmetrize(np.random.uniform(-1,1,(N,N))) #( 2*np.random.rand(N, N) - np.ones((N,N)) )
    np.fill_diagonal(A,0)
    sparseOld(A, rho)
    return (1 - epsilon/2.)*S + epsilon/2.*A

def get_connectivity_matrix_fortranStyle(N,rho,epsilon, inter, exter, mu, distrib):
    Ms = np.zeros((N,N))
    Ma = np.zeros((N, N))
    ...
    for i in range(0,N):
        for j in range(0,N):
            ...
            if distrib == 0:
                A1 = np.random.rand()*(exter-inter) + inter   
                A2 = np.random.rand()*(exter-inter) + inter
            if distrib == 1:
                A1 = truncnorm.rvs(inter, exter) + mu
                A2 = truncnorm.rvs(inter, exter) + mu
            if distrib == 2:
                A1 = np.random.lognormal()  
                A2 = np.random.lognormal()
            if distrib == 3:
                A1 = -np.random.lognormal()  
                A2 = -np.random.lognormal()                     
...
def getCyclesNX(C,N,typ,thr):
    ...
    lenghts = []
    lenghts = [float(len(loops[i])) for i in range(len(loops))]
    
    #print 'loops', loops
    #t2 = time.time()
    #print 'computed number of loops',t2-t1

    return loops, lenghts, G
...
if __name__ == "__main__":
    ...
    #test2()
    #test3()
    arguments = sys.argv[1:]     #### N, typ, epsilon, number of replicas, probability distribution, a, b, mu  ####

    N = int(arguments[0])
    typ = int(arguments[1])         # typ = 0 neurons with binary activation states {0,1}, typ = 1  neurons with states {-1,1}.
                                    # typ=1 --> {-1,1}    typ=0 --> {0,1} 
    eps = float(arguments[2])        
    thr = 0
    runNum = int(arguments[3])
    seed = 8888                    #int(np.random.rand() * 10000)
    pdf = int(arguments[4])        # 0 --> uniform ( [a,b] if a and b positive/negative, [b,a] if a,b both negative);    1 --> truncated gaussian in [a,b] shifted of c;    2 --> positive lognormal (a,b can be any);    3 --> negative lognormal (a,b can be any)     
    a = float(arguments[5])        # parameters that control the shift of the pdf
    b = float(arguments[6])
    c = float(arguments[7])

    np.random.seed(seed)
    rhos = [0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.8,0.85,0.9,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.99]
    ...
    for rho in rhos:
        ...
        for run in range(runNum):
            ...
            C = get_connectivity_matrix_fortranStyle(N=N,rho=rho,epsilon=eps, inter = a, exter = b, mu = c, distrib = pdf)
            ...
            loops, lenghts, G = getCyclesNX(C, N, typ, thr)
            print('loops', loops)
            num = len(loops)
            #print num
            numLoops.append(num)
            for i in range(len(lenghts)):
                numLenghts.append(lenghts[i])
        ...
        print(numLoops)
        avl = np.mean(numLenghts)
        avL = np.mean(numLoops)
        print('rho',rho,'acLoops',avL, 'acLenghts', avl)
        avCycles.append(avL)
        avLenghts.append(avl)

    plt.subplot(1,2,1)
    plt.title('Average Number of Cycles')
    plt.xlabel('rho')
    plt.ylabel('< C >')
    plt.plot(rhos,avCycles)
    plt.subplot(1,2,2)
    plt.title('Average Lenght of Cycles')
    plt.xlabel('rho')
    plt.ylabel('< L >')
    plt.plot(rhos, avLenghts)
    plt.show()
